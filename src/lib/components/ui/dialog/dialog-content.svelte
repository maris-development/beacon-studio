...
<Dialog.Portal {...portalProps}>
	...
	<DialogPrimitive.Content
		...
		class={cn(
			// A dialog never takes more than 90% of the window, in both directions.
			// Content past that height scrolls, so the footer stays reachable.
			"bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed left-[50%] top-[50%] z-50 grid max-h-[90vh] w-full max-w-[90vw] translate-x-[-50%] translate-y-[-50%] gap-4 overflow-y-auto rounded-lg border p-6 shadow-lg duration-200 sm:max-w-lg",
			className
		)}
		{...restProps}
	>
